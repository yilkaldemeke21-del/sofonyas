<!DOCTYPE html>
<html lang="am">
<head>
    <meta charset="UTF-8">
    <title>የሶፎንያስ ድር ገፅ</title>
    <link rel="stylesheet" href="sofonyas (1).css">
</head>
<body>
    <nav>
        <ul>
            <li><a href="https://t.me/+Bqeu85XkOu4yMzFk">Home</a></li>
            <li><a href="#about">About</a></li>
            <li><a href="student_login.php">exam center</a></li>
            <li><a href="#view">View</a></li>
            <li><a href="contact.html">Contact</a></li>
            <li><a href="contact.html">exam center</a></li>
        </ul>
    </nav>

    <header>
        <div id="logo">
            <img src="10 .jpg" alt="sofi" class="logo-img">
        </div>
        <h1>እንኳን ወደ ቤተ ገብርኤል በደህና መጡ!</h1>
        <p>ይህ የመቅደላ አምባ ዩኒቨርሲቲ ቤተ ገብርኤል ድር ገፅ ነው</p>
    </header>

    <div class="card">
        <h2>ስለ ቤተ ገብርኤል</h2>
        <p>ቤተ ገብርኤል በመቅደላ አምባ ዩኒቨርሲቲ በፈለገ ሰላም አዲስ አምባ ግቢ ጉባኤ የቤተሰብ እናት አባት አደረጃጀት ውስጥ አንዱና ተናፋቂው ቡድን ነው</p>
    </div>

    <div class="card">
        <h2>በዚህ ዌቭሳይት የሚካተቱ ትምህርቶች</h2>
        <ul>
            <li>ነገረ ሃይማኖት</li>
            <li>ስርዓተ ቤተ ክርስቲያን</li>
            <li>ነገረ ቅባት</li>
        </ul>
    </div>

    <section class="card contact-form">
        <h3>እንኳን ወደ ቤተ ገብርኤል በደህና መጡ!</h3>
        <form id="contactForm" action="register.php" method="post" novalidate>
            <label for="name">ስም</label>
            <input id="name" name="name" required placeholder="ስምዎን እዚህ ያስገቡ"><br><br>

            <label for="email">ኢሜይል</label>
            <input id="email" name="email" type="email" required placeholder="እባክዎ ኢሜይሎን ያስገቡ"><br><br>

            <label for="student_id">የተማሪ መለያ ቁጥር</label>
            <input type="text" id="student_id" name="student_id" placeholder="እባክዎ መለያ ቁጥር ያስገቡ" required><br><br>

            <label for="password">ፓስወርድ</label>
            <input id="password" name="password" type="password" placeholder="ፓስወርድ ያስገቡ" required><br><br>

            <div class="controls"><br>
                <button type="submit" class="button">ግባ</button>
                <button type="reset" class="button">አጥፋ</button>
            </div>
        </form>
        <div id="formMsg" class="small" aria-live="polite" style="margin-top:10px"></div>
    </section>

    <div class="card">
        <h2>እባክዎ ያነጋግሩኝ</h2>
        <p>Email: yilkaldemeke21@gmail.com</p>
    </div>

    <footer>
        <p><b>ይህ ዌቭሳይት የተሰራው በዲ/ን ሶፎንያስ ደመቀ (ወ/ጊዮርጊስ) ነው።</b></p>
    </footer>
</body>
</html>
